...
4. Update policy control (enable/disable automatic updates)
5. Repository management (local mirror configuration)
6. Internet access policy enforcement (allow/deny)
7. Policy management (unified policy status and reset)
8. Update execution (apt + fwupd, with dry-run support)
...
# Make sibling modules (apt_policy, fwupd_policy, etc.) importable
sys.path.insert(0, str(script_dir))
# Wire in the shared common/ module
...
# Lazy-import policy modules so missing files surface as clear errors
import apt_policy
import fwupd_policy
import internet_policy
import repo_manager
...
VERSION = "2.0.0"  # Bumped for Update Control Plane feature

_FUNCTIONAL_AREA = "controlled_sw_fw_updates"
_TOOL            = "update_control_plane"
STATE_FILE = Path(
    f"/var/lib/dgx_spark_management/{_FUNCTIONAL_AREA}/{_TOOL}/state.json"
)
...
def load_state(path: Path = STATE_FILE) -> Dict[str, Any]:
    """Load persisted state from JSON file; return empty dict on missing/corrupt file."""
    if not path.exists():
        return {}
    try:
...
def merge_state(updates: Dict[str, Any], path: Path = STATE_FILE) -> Dict[str, Any]:
    """Load, merge updates, save, and return the new state."""
...
def cmd_status(args, config: Dict) -> Dict:
    ...
    data = {
        ...
        "policy": {
            "updates_enabled": state.get("updates_enabled", True),
        ...
        "apt_timers": apt_policy.timers_status(),
        "fwupd_timer": fwupd_policy.get_timer_status(),
    }
...
# ============================================================================
# Updates Commands  (updates status / enable / disable)
# ============================================================================

def cmd_updates_status(args, config: Dict) -> Dict:
    """Report whether automatic updates are enabled."""
    state = load_state()
    enabled = state.get("updates_enabled", True)
    return format_envelope(True, {
        "updates_enabled": enabled,
        "apt_timers":      apt_policy.timers_status(),
        "fwupd_timer":     fwupd_policy.get_timer_status(),
        "changed_at":      state.get("updates_enabled_changed_at"),
    })


def cmd_updates_enable(args, config: Dict) -> Dict:
    """Enable automatic updates (APT timers + fwupd timer)."""
    is_root, err = require_root()
    if not is_root:
        return format_envelope(False, None, [err])

    errors: List[str] = []
    ok1, errs1 = apt_policy.enable_timers()
    errors.extend(errs1)
    ok2, errs2 = fwupd_policy.enable_timer()
    errors.extend(errs2)

    if errors:
        return format_envelope(False, None, errors)

    state = merge_state({
        "updates_enabled": True,
        "updates_enabled_changed_at": _now_utc(),
    })
    return format_envelope(True, {
        "updates_enabled": True,
        "apt_timers":      apt_policy.timers_status(),
...
def cmd_updates_disable(args, config: Dict) -> Dict:
    """Disable automatic updates (stop/disable APT + fwupd timers)."""
    is_root, err = require_root()
    if not is_root:
        return format_envelope(False, None, [err])

    errors: List[str] = []
    ok1, errs1 = apt_policy.disable_timers()
    errors.extend(errs1)
    ok2, errs2 = fwupd_policy.disable_timer()
    errors.extend(errs2)
...
# ============================================================================
# Repo Commands  (repo status / set / reset)
# ============================================================================

def cmd_repo_status(args, config: Dict) -> Dict:
    """Report current APT and fwupd repository configuration."""
    state = load_state()
    data = repo_manager.get_status(apt_policy, fwupd_policy, state)
    return format_envelope(True, data)


def cmd_repo_set(args, config: Dict) -> Dict:
    """Configure local mirror URL(s) for APT and/or fwupd."""
    is_root, err = require_root()
    if not is_root:
        return format_envelope(False, None, [err])

    mirror_url = getattr(args, "mirror_url", None)
    fwupd_url  = getattr(args, "fwupd_url", None)

    if not mirror_url and not fwupd_url:
        return format_envelope(False, None,
            ["Provide --mirror-url and/or --fwupd-url"])

    ok, errors, state_update = repo_manager.set_repo(
        apt_policy, fwupd_policy, mirror_url, fwupd_url
    )
    if not ok:
        return format_envelope(False, None, errors)

    state_update["repo_set_at"] = _now_utc()
    state = merge_state(state_update)
    return format_envelope(True, {
        "local_mirror_url": state.get("local_mirror_url"),
        "fwupd_remote_url": state.get("fwupd_remote_url"),
        "repo_set_at":      state.get("repo_set_at"),
    }, errors)


def cmd_repo_reset(args, config: Dict) -> Dict:
    """Remove all DGX Spark-managed repo configuration and re-enable sources."""
    is_root, err = require_root()
    if not is_root:
        return format_envelope(False, None, [err])

    ok, errors, state_update = repo_manager.reset_repo(apt_policy, fwupd_policy)
    if not ok:
        return format_envelope(False, None, errors)

    state_update["repo_reset_at"] = _now_utc()
    merge_state(state_update)
    return format_envelope(True, {"reset": True, "repo_reset_at": state_update["repo_reset_at"]}, errors)


# ============================================================================
# Internet Commands  (internet status / allow / deny)
# ============================================================================

def cmd_internet_status(args, config: Dict) -> Dict:
    """Report current internet access policy for updates."""
    state = load_state()
    data = internet_policy.get_status(state)
    data["apt_sources"] = apt_policy.get_repo_status()
    return format_envelope(True, data)


def cmd_internet_allow(args, config: Dict) -> Dict:
    """Allow internet access for updates (re-enable disabled APT sources)."""
    is_root, err = require_root()
    if not is_root:
        return format_envelope(False, None, [err])

    ok, errors = internet_policy.apply_allow(apt_policy, fwupd_policy)
    if not ok:
        return format_envelope(False, None, errors)

    ts = _now_utc()
    merge_state(internet_policy.state_update_for_allow("spark_updatectl", ts))
    return format_envelope(True, {
        "internet_mode": "allow",
        "changed_at": ts,
        "apt_sources": apt_policy.get_repo_status(),
    }, errors)


def cmd_internet_deny(args, config: Dict) -> Dict:
    """Deny internet access for updates (disable non-DGX APT sources)."""
    is_root, err = require_root()
    if not is_root:
        return format_envelope(False, None, [err])

    ok, errors = internet_policy.apply_deny(apt_policy, fwupd_policy)
    if not ok:
        return format_envelope(False, None, errors)

    ts = _now_utc()
    merge_state(internet_policy.state_update_for_deny("spark_updatectl", ts))
    return format_envelope(True, {
        "internet_mode": "deny",
        "changed_at": ts,
        "apt_sources": apt_policy.get_repo_status(),
...
# ============================================================================
# Policy Commands  (policy status / set / reset)
# ============================================================================

def cmd_policy_status(args, config: Dict) -> Dict:
    """Report full unified policy state."""
    state = load_state()
    return format_envelope(True, {
        "updates_enabled":   state.get("updates_enabled", True),
        "internet_mode":     state.get("internet_mode", "allow"),
        ...
        "fwupd_timer":       fwupd_policy.get_timer_status(),
        "apt_sources":       apt_policy.get_repo_status(),
        "fwupd_remotes":     fwupd_policy.get_remote_status(),
        "state_file":        str(STATE_FILE),
    })


def cmd_policy_set(args, config: Dict) -> Dict:
    """Set one or more policy values (key=value pairs)."""
    is_root, err = require_root()
    if not is_root:
        return format_envelope(False, None, [err])

    updates: Dict[str, Any] = {}
    kv_pairs = getattr(args, "kv", []) or []
    errors: List[str] = []

    ALLOWED_KEYS = {
        "updates_enabled", "internet_mode", "local_mirror_url", "fwupd_remote_url"
    }

    for pair in kv_pairs:
        if "=" not in pair:
            errors.append(f"Invalid key=value pair: {pair!r}")
            continue
        k, _, v = pair.partition("=")
        if k not in ALLOWED_KEYS:
            errors.append(f"Unknown policy key: {k!r} (allowed: {sorted(ALLOWED_KEYS)})")
            continue
        # Type coerce booleans
        if k == "updates_enabled":
            if v.lower() in ("true", "1", "yes"):
                updates[k] = True
            elif v.lower() in ("false", "0", "no"):
                updates[k] = False
            else:
                errors.append(f"Invalid boolean value for {k}: {v!r}")
                continue
        else:
            updates[k] = v
    ...
    updates["policy_set_at"] = _now_utc()
    state = merge_state(updates)
    return format_envelope(True, {"applied": updates, "state_file": str(STATE_FILE)})


def cmd_policy_reset(args, config: Dict) -> Dict:
    """Reset all policy to defaults (updates enabled, internet allow, no custom mirrors)."""
    is_root, err = require_root()
    if not is_root:
        return format_envelope(False, None, [err])
    ...
    # Re-enable internet sources
    ok3, errs3 = internet_policy.apply_allow(apt_policy, fwupd_policy)
    errors.extend(errs3)

    # Remove managed repo files
    ok4, errs4, _ = repo_manager.reset_repo(apt_policy, fwupd_policy)
    errors.extend(errs4)
    ...
    merge_state({
        ...
        "policy_reset_at":            ts,
        "updates_enabled_changed_at": ts,
        "internet_mode_changed_at":   ts,
    })

    if errors:
        return format_envelope(False, None, errors)
    return format_envelope(True, {"reset": True, "policy_reset_at": ts})


# ============================================================================
# Update Commands  (update now / check)
# ============================================================================

def cmd_update_now(args, config: Dict) -> Dict:
    """Run apt-get upgrade + fwupdmgr update immediately."""
    is_root, err = require_root()
    if not is_root:
        return format_envelope(False, None, [err])

    errors: List[str] = []
    results: Dict[str, Any] = {"dry_run": False}

    # APT
    ok_apt, apt_out, apt_errs = apt_policy.run_apt_update(dry_run=False)
    errors.extend(apt_errs)
    results["apt_output"] = apt_out
    results["apt_ok"] = ok_apt

    # fwupd
    ok_fw, fw_out, fw_errs = fwupd_policy.run_fwupd_update(dry_run=False)
    errors.extend(fw_errs)
    results["fwupd_output"] = fw_out
    results["fwupd_ok"] = ok_fw

    ts = _now_utc()
    results["completed_at"] = ts
    merge_state({"last_update_at": ts})

    ok = ok_apt and ok_fw
    return format_envelope(ok, results, errors)


def cmd_update_check(args, config: Dict) -> Dict:
    """Dry-run: check for available updates without installing."""
    errors: List[str] = []
    results: Dict[str, Any] = {"dry_run": True}

    # APT dry-run
    ok_apt, apt_out, apt_errs = apt_policy.run_apt_update(dry_run=True)
    errors.extend(apt_errs)
    results["apt_output"] = apt_out
    results["apt_ok"] = ok_apt

    # fwupd dry-run
    ok_fw, fw_out, fw_errs = fwupd_policy.run_fwupd_update(dry_run=True)
    errors.extend(fw_errs)
    results["fwupd_output"] = fw_out
    results["fwupd_ok"] = ok_fw

    ts = _now_utc()
    results["checked_at"] = ts
    # Only save check timestamp if we have write access
    try:
        merge_state({"last_check_at": ts})
    except Exception:
        pass  # Read-only is acceptable for check
...
def cmd_self_test(args, config: Dict) -> Dict:
    """Run internal sanity checks and return a test report."""
    checks: List[Dict] = []

    def check(name: str, passed: bool, detail: str = "") -> None:
        checks.append({"name": name, "passed": passed, "detail": detail})

    # Check: state file directory is writable or doesn't exist yet
    state_dir = STATE_FILE.parent
    dir_writable = not state_dir.exists() or os.access(str(state_dir), os.W_OK)
    check("state_dir_writable", dir_writable, str(state_dir))

    # Check: apt-get is available
    check("apt_get_available", shutil.which("apt-get") is not None)

    # Check: systemctl is available
    check("systemctl_available", shutil.which("systemctl") is not None)

    # Check: fwupdmgr presence (non-fatal)
    check("fwupdmgr_available", fwupd_policy.is_fwupd_available(),
          "fwupd not installed — firmware updates unavailable")

    # Check: APT sources dir exists
    check("apt_sources_dir_exists", apt_policy.APT_SOURCES_DIR.exists(),
          str(apt_policy.APT_SOURCES_DIR))

    # Check: state file loads without error
    state = load_state()
    check("state_loads_ok", isinstance(state, dict))

    all_passed = all(c["passed"] for c in checks)
    return format_envelope(all_passed, {
        "checks": checks,
        "total": len(checks),
...
def main() -> int:
    ...
    # ── updates command ──────────────────────────────────────────────────────
    updates_parser = subparsers.add_parser("updates", help="Automatic update policy (enable/disable)")
    updates_sub = updates_parser.add_subparsers(dest="updates_cmd", help="Updates subcommand")
    updates_sub.add_parser("status", help="Show automatic update policy status")
    updates_sub.add_parser("enable", help="Enable automatic updates (APT + fwupd timers)")
    updates_sub.add_parser("disable", help="Disable automatic updates (stop/disable timers)")

    # ── repo command ─────────────────────────────────────────────────────────
    repo_parser = subparsers.add_parser("repo", help="Repository configuration")
    repo_sub = repo_parser.add_subparsers(dest="repo_cmd", help="Repo subcommand")
    repo_sub.add_parser("status", help="Show current repo configuration")
    repo_set_parser = repo_sub.add_parser("set", help="Configure local mirror URL(s)")
    repo_set_parser.add_argument("--mirror-url", metavar="URL",
        help="APT local mirror base URL (e.g. http://192.168.1.10/ubuntu)")
    repo_set_parser.add_argument("--fwupd-url", metavar="URL",
        help="fwupd metadata mirror URL (e.g. http://192.168.1.10/fwupd/)")
    repo_sub.add_parser("reset", help="Remove all DGX Spark-managed repo config")

    # ── internet command ─────────────────────────────────────────────────────
    internet_parser = subparsers.add_parser("internet", help="Internet access policy for updates")
    internet_sub = internet_parser.add_subparsers(dest="internet_cmd", help="Internet subcommand")
    internet_sub.add_parser("status", help="Show internet access policy")
    internet_sub.add_parser("allow", help="Allow internet access for updates")
    internet_sub.add_parser("deny", help="Deny internet access; enforce local-mirror-only mode")

    # ── policy command ───────────────────────────────────────────────────────
    policy_parser = subparsers.add_parser("policy", help="Unified policy management")
    policy_sub = policy_parser.add_subparsers(dest="policy_cmd", help="Policy subcommand")
    policy_sub.add_parser("status", help="Show full policy state")
    policy_set_parser = policy_sub.add_parser("set",
        help="Set policy key=value pairs (keys: updates_enabled, internet_mode, local_mirror_url, fwupd_remote_url)")
    policy_set_parser.add_argument("kv", nargs="+", metavar="KEY=VALUE",
        help="Policy key=value pairs")
    policy_sub.add_parser("reset", help="Reset all policy to defaults")

    # ── update command ───────────────────────────────────────────────────────
    update_parser = subparsers.add_parser("update", help="Trigger package/firmware updates")
    update_sub = update_parser.add_subparsers(dest="update_cmd", help="Update subcommand")
    update_sub.add_parser("now", help="Run apt upgrade + fwupdmgr update immediately (requires sudo)")
    update_sub.add_parser("check", help="Dry-run: check for available updates without installing")
    ...
    # Handle --self-test before command routing
    if getattr(args, "self_test", False):
        result = cmd_self_test(args, config)
    ...
    try:
        if args.command == "status":
            ...
            if args.updates_cmd == "status":
                result = cmd_updates_status(args, config)
                command_name = "updates_status"
            elif args.updates_cmd == "enable":
                result = cmd_updates_enable(args, config)
                command_name = "updates_enable"
            elif args.updates_cmd == "disable":
                result = cmd_updates_disable(args, config)
                command_name = "updates_disable"
            else:
                result = format_envelope(False, None, ["Unknown updates subcommand"])

        elif args.command == "repo":
            if args.repo_cmd == "status":
                result = cmd_repo_status(args, config)
                command_name = "repo_status"
            elif args.repo_cmd == "set":
                result = cmd_repo_set(args, config)
                command_name = "repo_set"
            elif args.repo_cmd == "reset":
                result = cmd_repo_reset(args, config)
                command_name = "repo_reset"
            else:
                result = format_envelope(False, None, ["Unknown repo subcommand"])

        elif args.command == "internet":
            if args.internet_cmd == "status":
                result = cmd_internet_status(args, config)
                command_name = "internet_status"
            elif args.internet_cmd == "allow":
                result = cmd_internet_allow(args, config)
                command_name = "internet_allow"
            elif args.internet_cmd == "deny":
                result = cmd_internet_deny(args, config)
                command_name = "internet_deny"
            else:
                result = format_envelope(False, None, ["Unknown internet subcommand"])

        elif args.command == "policy":
            if args.policy_cmd == "status":
                result = cmd_policy_status(args, config)
                command_name = "policy_status"
            elif args.policy_cmd == "set":
                result = cmd_policy_set(args, config)
                command_name = "policy_set"
            elif args.policy_cmd == "reset":
                result = cmd_policy_reset(args, config)
                command_name = "policy_reset"
            else:
                result = format_envelope(False, None, ["Unknown policy subcommand"])

        elif args.command == "update":
            if args.update_cmd == "now":
                result = cmd_update_now(args, config)
                command_name = "update_now"
            elif args.update_cmd == "check":
                result = cmd_update_check(args, config)
                command_name = "update_check"
            else:
                result = format_envelope(False, None, ["Unknown update subcommand"])
